...
from lender_intel.errors import ConfigurationError, ExtractionError
...
""", encoding="utf-8")
    ...
    def test_demo_cli_writes_complete_bundle(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            for name in ("records.json", "comparability_matrix.json", "calculations.json", "validation_report.json", "analyst_brief.html", "demo_manifest.json"):
                self.assertTrue((output / name).is_file(), name)
            self.assertIn("Cross-lender comparison", (output / "analyst_brief.md").read_text(encoding="utf-8"))
